...
def battle_report(attacker, defender, attacker_loss, defender_loss, plunder=None, building_damage=None):
    ...
    for k in attacker_loss.keys():
        body += f'{attacker_loss[k]}:{gr.unit_types[k]}'
    body += '\n\nGubici branioca'
    for k in defender_loss.keys():
        body += f'{defender_loss[k]}:{gr.unit_types[k]}'
    if plunder is not None:
        gold_p = plunder['gold']
        wood_p = plunder['wood']
        stone_p = plunder['stone']
        body += f'\n\n Napadač je osvojio {gold_p} zlata, {wood_p} drva i {stone_p} kamena'
    if building_damage is not None:
        body += '\n\nBranioc je primio ovu štetu:'
        for k in building_damage:
            body += f'{gr.building_types[k]}'
...
defenseBonus = [1, 1.1, 1.2, 1.3, 1.4, 1.5, 1.6]
...
class battleEvent(cityEvent):
    ...
    def execute(self):
        ...
        DPower = battleEvent.armyPower(defender, attacker) * defenseBonus[wallLvl]
